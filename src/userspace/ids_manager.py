...
class DeploymentController:
    """Controls the eBPF program deployment to the kernel."""
    ...
    def prepare_batch(self, batch_num: int) -> bool:
        """Prepare eBPF code for a specific batch."""
    ...
    def deploy_batch_progressive(self, start_batch: int = 0, num_batches: int = 1) -> bool:
        """Deploy batches progressively."""
        print("Progressive Deployment Starting...")
        print(f"Starting from batch {start_batch}, loading {num_batches} batches.")
    ...
    def export_all_codes(self) -> bool:
    ...
    def deploy_test_batch(self, batch_num: int, test_nmap: bool = False) -> bool:
        """Prepares a single batch and provides a test command."""
        print(f"Preparing test for batch {batch_num}...")
        if self.prepare_batch(batch_num):
            if test_nmap:
                test_cmd = self.rule_manager.get_test_command(batch_num)
                if test_cmd:
                    print("You can now run the following command in another terminal to test:")
                    print(test_cmd)
            return True
        return False
...
USAGE EXAMPLES:
---------------
1.  Test with a single batch (e.g., batch 0) and generate an nmap test command:
    python3 ids_manager.py --test-batch 0

2.  Export all generated eBPF C code to the 'generated_ebpf' directory:
    python3 ids_manager.py --export

3.  Show the current deployment status:
    python3 ids_manager.py --status

KEY FEATURES:
-------------
- OOP-based modular design.
- Progressive batch deployment of rules.
- Automatic nmap test command generation for validation.
- Resolution of HOMENET and EXTERNALNET variables.
- Clean separation of concerns between rule management, code generation, and deployment.

TESTING GUIDANCE:
-----------------
1.  Start by testing a single batch (e.g., --test-batch 0).
2.  The system will generate a corresponding nmap command.
3.  Run the nmap command from another terminal to trigger the rules in that batch.
4.  Check 'ids_alerts.log' for detection alerts.
5.  If successful, proceed to test the next batch or export all code.
"""
    print(help_text)
...
if __name__ == "__main__":
    ...
    # Use the new IDSConfig and DeploymentController
    config = IDSConfig()
    manager = DeploymentController(config)

    # Initialize the system (loads rules, etc.)
    if not manager.initialize():
        print("Failed to initialize IDS Manager. Exiting.")
        sys.exit(1)

    parser = argparse.ArgumentParser(description="eBPF IDS Manager")
    parser.add_argument("--test-batch", type=int, help="Test a specific batch number.")
    parser.add_argument("--export", action="store_true", help="Export all code without deployment.")
    ...
    if args.help_usage:
        ...
        if not manager.export_all_codes():
            print("Export failed.")
    elif args.status:
        status = manager.get_deployment_status()
        print(json.dumps(status, indent=2))
    elif args.test_batch is not None:
        if not manager.deploy_test_batch(args.test_batch, test_nmap=True):
            print(f"Failed to prepare test for batch {args.test_batch}.")
    else:
        parser.print_help()
